...
const raisedAmount = GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0);
const totalGames = GAMES_JSON.length;
const numberOfUnfundedGames = GAMES_JSON.filter(game => game.pledged < game.goal).length;

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `
A total of $${raisedAmount.toLocaleString()} has been raised for ${totalGames} games. Currently, ${numberOfUnfundedGames === 1 ? 'one game remains' : `${numberOfUnfundedGames} games remain`} unfunded. We need your help to fund these amazing games!
`;
// create a new DOM element containing the template string and append it to the description container
const unfundedParagraph = document.createElement("p");
unfundedParagraph.textContent = displayStr;
...
const firstGameElem = document.createElement("div");
firstGameElem.innerHTML = `
    <h3>${firstGame.name}</h3>
`;
firstGameContainer.appendChild(firstGameElem);
// do the same for the runner up item
const secondGameElem = document.createElement("div");
secondGameElem.innerHTML = `
